...
async def download_visitas_cad():
    ...
    async with async_playwright() as p:
        ...
        # 4. Natureza Geral -> OCORRÊNCIA SEM ILICITUDE
        try:
            log("Selecionando Natureza: OCORRÊNCIA SEM ILICITUDE...")
            # Aguarda a lista de naturezas carregar (geralmente carrega após selecionar Órgão/Unidade)
            await f_filtros.wait_for_selector('#SC_ocor_id_ocor_grup_fk_orig option', timeout=10000)
            options = await f_filtros.locator('#SC_ocor_id_ocor_grup_fk_orig option').all()
            val_ilicitude = None
            for opt in options:
                text = await opt.inner_text()
                if "OCORRÊNCIA SEM ILICITUDE" in text.upper():
                    val_ilicitude = await opt.get_attribute('value')
                    break
            
            if val_ilicitude:
                log(f"Valor encontrado para Natureza: {val_ilicitude}")
                await f_filtros.locator('#SC_ocor_id_ocor_grup_fk_orig').select_option(value=val_ilicitude)
                await f_filtros.locator(f'#SC_ocor_id_ocor_grup_fk_orig option[value="{val_ilicitude}"]').dblclick()
                log("Natureza selecionada. Aguardando Tipicidades (AJAX)...")
                await asyncio.sleep(3) # Tempo para o scriptcase carregar o segundo select
            else:
                log("Aviso: 'OCORRÊNCIA SEM ILICITUDE' não encontrada na Natureza Geral.")
        except Exception as e:
            log(f"Erro Natureza: {e}")

        # 5. Tipicidade Geral -> 3 Opções
        try:
            log("Selecionando Tipicidades solicitadas...")
            # Aguarda as tipicidades carregarem
            await f_filtros.wait_for_selector('#SC_ocor_id_ocor_sgrup_fk_orig option', timeout=15000)
            
            targets = [
                "VISITA COMUNITÁRIA",
                "VISITA COMUNITÁRIA / MARIA DA PENHA",
                "VISITA PREVENTIVA"
            ]
            
            for target in targets:
                found = False
                options = await f_filtros.locator('#SC_ocor_id_ocor_sgrup_fk_orig option').all()
                for opt in options:
                    text = (await opt.inner_text()).strip().upper()
                    if target.upper() == text:
                        val = await opt.get_attribute('value')
                        log(f"Selecionando Tipicidade: {text} (Value: {val})")
                        await f_filtros.locator('#SC_ocor_id_ocor_sgrup_fk_orig').select_option(value=val)
                        await f_filtros.locator(f'#SC_ocor_id_ocor_sgrup_fk_orig option[value="{val}"]').dblclick()
                        await asyncio.sleep(0.5)
                        found = True
                        break
                if not found:
                    log(f"Aviso: Tipicidade '{target}' não encontrada na lista.")
            
        except Exception as e:
            log(f"Erro Tipicidade: {e}")
        ...
        for frame in v_page.frames:
            if "cad_grid_tb_ocor_consulta_com_cadastro.php" in frame.url and "fil" not in frame.url:
                f_grid = frame
                break
        if not f_grid: f_grid = v_page
            
        await f_grid.screenshot(path=os.path.join(WORKSPACE, 'debug_cad_resultados_visitas.png'))
        ...
        try:
            ...
            path_xls = os.path.join(DESTINO_DIR, "Visita Comunitária 2026.xls")
            await download.save_as(path_xls)
            log(f"✅ Visitas baixadas com sucesso: {path_xls}")
            
        except Exception as e:
            log(f"Aviso: Fallback para download na página principal... ({e})")
            try:
                async with v_page.expect_download(timeout=10000) as download_info:
                    await f_grid.click('text=Baixar')
                download = await download_info.value
                path_xls = os.path.join(DESTINO_DIR, "Visita Comunitária 2026.xls")
                await download.save_as(path_xls)
                log(f"✅ Visitas baixadas via fallback: {path_xls}")
            except: log("Erro final: Não foi possível baixar.")

        await browser.close()

if __name__ == "__main__":
    asyncio.run(download_visitas_cad())
